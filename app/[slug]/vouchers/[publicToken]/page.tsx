...
export async function generateMetadata({ params }: VoucherPageProps): Promise<Metadata> {
  ...
  return {
    title: resort ? `Booking voucher | ${resort.name}` : "Booking voucher | Travelseed",
    description: resort ? `Booking confirmation for ${resort.name}.` : "Booking confirmation.",
  };
}
    ...
    <main className="min-h-screen" style={{ backgroundColor: design.colors.page, color: design.colors.text }}>
      ...
      <section className="px-5 py-10 sm:px-6 lg:py-16">
        <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em]" style={{ color: design.colors.accent }}>Booking voucher</p>
            <h1 className="mt-3 text-4xl font-semibold leading-tight sm:text-5xl">Your stay is confirmed</h1>
            <p className="mt-4 max-w-xl text-base leading-7 opacity-75">Please keep this confirmation available for check-in. Contact the property if any booking details need to change.</p>
          ...
          <article className="overflow-hidden rounded-[28px] border border-black/10 bg-white shadow-[0_24px_90px_rgba(30,25,18,0.12)]">
            <div className="p-6 sm:p-8" style={{ backgroundColor: design.colors.primary, color: design.colors.buttonText }}>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <p className="text-sm uppercase tracking-[0.18em] opacity-70">Voucher</p>
            ...
            <div className="grid gap-6 p-6 sm:p-8">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#8a7560]">Guest</p>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <DetailCard label="Check-in" value={formatDate(voucher.check_in)} />
                <DetailCard label="Check-out" value={formatDate(voucher.check_out)} />
                <DetailCard label="Guests" value={voucher.guests ? String(voucher.guests) : "TBC"} />
              </div>

              <div className="grid gap-3 sm:grid-cols-2">
                <DetailCard label="Offer" value={voucher.offer_title || "Direct booking"} />
                <DetailCard label="Room" value={voucher.room_label || "Assigned by property"} />
              </div>
...
function formatDateTime(value: string | null) {
  if (!value) {
    return "after property confirmation";
  }
