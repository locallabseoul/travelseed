  ...
  return (
    <div className="grid gap-6">
      <Panel>
        ...
        <p className="mt-2 text-sm leading-6 text-[#6f7b74]">Tree is built for direct booking operations with enough capacity for growing hospitality businesses.</p>
      </Panel>

      <div className="grid gap-4 xl:grid-cols-4">
        {planOptions.map((plan) => (
          <Panel key={plan.name} className={plan.current ? "ring-2 ring-[#2d6b50]" : ""}>
            ...
            <div className="mt-6">{plan.current ? <SecondaryButton>Current plan</SecondaryButton> : <PrimaryButton>Select plan</PrimaryButton>}</div>
          </Panel>
        ))}
      </div>
    </div>
