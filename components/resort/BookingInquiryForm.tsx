    ...
    <>
      ...
      {open && mounted ? createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-[#10241f]/70 px-4 py-6 backdrop-blur-sm">
          <button type="button" aria-label="Close inquiry form" onClick={() => setOpen(false)} className="absolute inset-0 cursor-default" />
          <div className="relative max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 text-[#18352f] shadow-[0_28px_100px_rgba(16,36,31,0.34)] sm:p-6">
            <div className="flex items-start justify-between gap-4 border-b border-[#eadfce] pb-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#72815e]">{category.inquiry.eyebrow}</p>
                <h2 className="mt-2 text-2xl font-semibold text-[#18352f]">{category.inquiry.title}</h2>
                <p className="mt-2 text-sm leading-6 text-[#6f7b74]">
...
export function BookingInquiryForm({
  ...
  async function handleBookingClick() {
    ...
    const payload = JSON.stringify({
      ...
      source,
      inquiry: {
        guestName: form.guestName.trim() || (accommodation ? "Website WhatsApp guest" : "Website WhatsApp customer"),
  ...
  return (
    <div className="grid gap-4">
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Your name" value={form.guestName} placeholder={category.inquiry.namePlaceholder} onChange={(guestName) => setForm((current) => ({ ...current, guestName }))} />
      ...
      {accommodation ? (
        ...
        <div className="grid gap-3">
          <Field label={category.inquiry.requestLabel} value={form.request} placeholder={category.inquiry.requestPlaceholder} textarea onChange={(request) => setForm((current) => ({ ...current, request }))} />
          <Field label={category.inquiry.preferredTimeLabel} value={form.preferredTime} placeholder={category.inquiry.preferredTimePlaceholder} onChange={(preferredTime) => setForm((current) => ({ ...current, preferredTime }))} />
        </div>
      )}
      <a
        href={bookingUrl}
...
function buildBookingMessage(resort: Resort, form: typeof emptyForm) {
  const category = businessCategoryFromType({ type: resort.type, templateId: resort.template_id });
  const base = resort.booking_message_template || category.defaultBookingMessage(resort.name);
  const accommodation = category.id === "accommodation";
  const values: Record<string, string> = {
    ...
    "Preferred date or time": form.preferredTime,
  };
  ...
  return [...lines, "", `${accommodation ? "Guest Name" : "Name"}: ${form.guestName || "-"}`, `Contact: ${form.guestContact || "-"}`].join("\n");
}
...
function Field({
  ...
  return (
    <label className="grid gap-2 text-sm font-semibold text-[#18352f]">
      {label}
      {textarea ? (
        <textarea value={value} placeholder={placeholder} onChange={(event) => onChange(event.target.value)} className="min-h-24 rounded-xl border border-[#d8cebb] bg-white px-3 py-3 text-sm outline-none focus:border-[#18352f]" />
      ) : (
